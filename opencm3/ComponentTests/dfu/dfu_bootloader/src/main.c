...
int main(void)
{
	...
	// een input waarmee je dFU mode kan forceren
	rcc_periph_clock_enable(RCC_GPIOA);
	rcc_periph_clock_enable(RCC_GPIOC);

	// PA10 low -> jump to app, PA10 high -> dfu bootloader
	...
	usbd_dev = usbd_init(&st_usbfs_v1_usb_driver, &dev, &config, usb_strings, 4, usbd_control_buffer, sizeof(usbd_control_buffer));usbd_register_set_config_callback(usbd_dev, usbdfu_set_config);

	gpio_clear(GPIOC, GPIO13); // led aan
